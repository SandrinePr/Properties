      ...
      <div className="pd-container">
        ...
        <div className="pd-grid-wrapper">
          <div className="pd-grid-main" onClick={() => setPhotoIndex(0)}>
            {allImages[0] && <img src={allImages[0]} alt="Hero" referrerPolicy="no-referrer" />}
          </div>
          
          <div className="pd-grid-side">
            <div className="pd-side-item" onClick={() => setPhotoIndex(1)}>
              {allImages[1] ? <img src={allImages[1]} alt="Detail 1" referrerPolicy="no-referrer" /> : <div className="pd-placeholder"></div>}
            </div>
            
            <div className="pd-side-item" onClick={() => setPhotoIndex(2)}>
              {allImages[2] ? <img src={allImages[2]} alt="Detail 2" referrerPolicy="no-referrer" /> : <div className="pd-placeholder"></div>}
              {allImages.length > 3 && (
                <div className="pd-image-badge"><span>+{allImages.length - 2} foto's</span></div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="pd-info-card">
          ...
          <div className="pd-stats">
            <div className="pd-stat"><span className="label">PRIJS</span><span className="value">€ {Number(acf.price).toLocaleString('nl-NL')}</span></div>
            <div className="pd-stat"><span className="label">OPPERVLAKTE</span><span className="value">{acf.square_footage} m²</span></div>
            <div className="pd-stat"><span className="label">SLAAPKAMERS</span><span className="value">{acf.bedrooms}</span></div>
            <div className="pd-stat"><span className="label">BOUWJAAR</span><span className="value">{acf.construction_year}</span></div>
          </div>
          <div className="pd-desc-box" style={{marginTop: '30px'}}>
             <p>{acf.description}</p>
          </div>
        </div>
      </div>
